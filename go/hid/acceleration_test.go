...
func TestAcceleration_FastClicks_Accelerates(t *testing.T) {
	a := NewAccelerationHandler(0.2, 0.15, []int{1, 1, 2, 2, 3})
	// 1st click resets to 1. Then list[count]: list[1]=1, list[2]=2, list[3]=2, list[4]=3
	a.CalculateSpeed(1.0, 1)           // reset, delta=1
	a.CalculateSpeed(1.05, 1)          // list[1]=1
	a.CalculateSpeed(1.10, 1)          // list[2]=2
	a.CalculateSpeed(1.15, 1)          // list[3]=2
	delta := a.CalculateSpeed(1.20, 1) // list[4]=3
	if delta != 3 {
...
func TestAcceleration_BeyondListLength_UsesLastValue(t *testing.T) {
	...
	// reset=1, list[1]=2, list[2]=3, then last element repeats
	a.CalculateSpeed(1.0, 1)           // reset
	a.CalculateSpeed(1.05, 1)          // list[1]=2
	a.CalculateSpeed(1.10, 1)          // list[2]=3
	delta := a.CalculateSpeed(1.15, 1) // beyond len, last=3
	if delta != 3 {
